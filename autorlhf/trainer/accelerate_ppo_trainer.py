...
import autorlhf.utils.logging as logging
from autorlhf.data.accelerate_base_datatypes import PromptBatch
from autorlhf.data.configs import AutoRLHFConfig
from autorlhf.data.ppo_types import PPORLBatch, PPORLElement
from autorlhf.models.modeling_ppo import (
...
from autorlhf.pipeline.ppo_pipeline import PPORolloutStorage
from autorlhf.trainer import register_trainer
from autorlhf.trainer.accelerate_base_trainer import AccelerateRLTrainer
from autorlhf.utils import Clock, infinite_dataloader
from autorlhf.utils.modeling import RunningMoments, gather_dict, logprobs_of_labels
...
class AcceleratePPOTrainer(AccelerateRLTrainer):
    ...
    def __init__(self, config: AutoRLHFConfig, **kwargs):
    ...
    def get_arch(self, config: AutoRLHFConfig):
        """Get the model"""
        ...
        while len(ppo_rl_elements) < num_rollouts:
            ...
            if self.config.model.model_arch_type == "seq2seq":
                ...
                with torch.no_grad():
                    ...
                    # TODO(): When hydra model works need to also support generation on hydra head
                    if hasattr(self.model, "frozen_head") or self.model.peft_type:
                        ref_logits = self.model.forward_hydra(
                            all_tokens,
                            attention_mask=attention_mask,
