...
from autorlhf.data.method_configs import MethodConfig, get_method
...
class ModelConfig:
    ...
    :param peft_config: configuration for peft (Parameter Efficient Fine-Tuning library).
        ...
        (parameter-efficient fine-tuning was previously done in autorlhf with OpenDelta, but it is no longer supported)
...
class TrainConfig:
    ...
    :param pipeline: Pipeline to use for training. One of the registered pipelines present in autorlhf.pipeline
    :type pipeline: str

    :param trainer: Trainer to use for training. One of the registered trainers present in autorlhf.trainer
    :type trainer: str
...
@dataclass
class AutoRLHFConfig:
    """
    Top level config for autorlhf. Loads configs and can be converted to dictionary.
    """
    ...
    def load_yaml(cls, yml_fp: str):
        """
        Load yaml file as AutoRLHFConfig.
    ...
    def to_dict(self):
        """
        Convert AutoRLHFConfig to dictionary.
        """
        data = {
    ...
    def evolve(self, **kwargs) -> "AutoRLHFConfig":
        """
        Evolve AutoRLHFConfig with new parameters. Can update nested parameters.
        >>> config = autorlhf.data.default_configs.default_ilql_config()
        >>> config = config.evolve(method=dict(gamma=0.99, gen_kwargs=dict(max_new_tokens=100))
        >>> config.method.gamma
        0.99
        """
        return AutoRLHFConfig.from_dict(_merge_dicts(self.to_dict(), kwargs))

    @classmethod
    def from_dict(cls, config: Dict):
        """
        Convert dictionary to AutoRLHFConfig.
        """
        return cls(
